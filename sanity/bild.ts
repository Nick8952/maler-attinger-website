...
export interface SanityBildRoh {
  asset?: { _id: string; url: string; metadata?: { dimensions?: { width: number; height: number }; palette?: { dominant?: { background?: string } } } };
...
export const BILD_PROJEKTION = `{
  asset->{ _id, url, metadata { dimensions { width, height }, palette { dominant { background } } } },
  hotspot, crop, alt, bildunterschrift, farbton
}`;
...
export function sanityBild(roh: SanityBildRoh | null | undefined, fallbackAlt = ""): Bild | undefined {
  ...
  return {
    ...
    breite: dims.width,
    hoehe: dims.height,
    farbton: roh.farbton ?? roh.asset.metadata?.palette?.dominant?.background,
